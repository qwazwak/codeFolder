...
public class BlackJackPlayer
{
   ...
   public void split(Card card1, Card card2)
   {
      ...
      if (can_split)
      {
         ...
         BlackJackHand current = this.hands.get(index - 1);
	     BlackJackHand[] splitArr = current.split(card1, card2);
		 //remove the old hand and add the split hands in its place
	     this.hands.remove(index - 1);
	     this.hands.add(index - 1, splitArr[0]);
	     this.hands.add(index, splitArr[1]);
