...
void updateStats(studentArrayShell& dataShell){
	long* sumHolder = new long[dataShell.testCount];
	long singleHold = 0;
	...
	for (long i = 0; i < dataShell.testCount; i++) {
		cout << sumHolder[i] << "/" << dataShell.studentCount << endl;
		dataShell.testAvg[i] = static_cast<double>(sumHolder[i]) / static_cast<double>(dataShell.studentCount);
		cout << dataShell.testAvg[i] << endl;
