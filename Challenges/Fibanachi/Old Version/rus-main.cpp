...
int main (){
	...
	//Variables:
	intmax_t precisionValue;
	intmax_t userLimit;
	intmax_t writeToFileRate;
	
	
	
	
	do{
		cout << "Fibonacci Number Calculator - Starting with 0, 1" << endl;
		cout << "Enter the number of terms you want to calculate to?" << endl;
		cin >> userLimit;
		cin.clear();
		cin.ignore();
	}while(userLimit < 1 || cin.fail());
	do{
		cout << "Fibonacci Number Calculator - Starting with 0, 1" << endl;
		...
		cin.clear();
		cin.ignore();
	}while(precisionValue < 1 || cin.fail());
	
	cout << setprecision(precisionValue) << endl;
	fibFileOutput << setprecision(precisionValue) << endl;
	long double arrayOfFib[userLimit];
	arrayOfFib[0] = 0;
	arrayOfFib[1] = 1;
	fibFileOutput << arrayOfFib[0] << endl;
	fibFileOutput << arrayOfFib[1] << endl;
	//cout << arrayOfFib[0] << endl;
	//cout << arrayOfFib[1] << endl;
	for(intmax_t i = 2; i <= userLimit - 2; i = i + 1){
		arrayOfFib[i] = arrayOfFib[i - 1] + arrayOfFib[i - 2];
		fibFileOutput << arrayOfFib[i] << endl;
		//cout << arrayOfFib[i] << endl;
	}
	
	//for(intmax_t j = 1; j <= userLimit; j = j + 1){
