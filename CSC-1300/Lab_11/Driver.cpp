...
class deck {
	...
	public:
		...
		void customSeed(long inputSeed){
			seed = inputSeed;
		}
		void generateTimeSeed(){
			seed = time(NULL);
		}


		void fillDeck(long fillSize){
			size = fillSize;
			long optionsLeft = size;
			deckArray = new long[size];
			long* option = new long[size];
			long* optionsSwap = new long[size];
			for (long i = 0; i < size; i++) {
				option[i] = i;
			}
			for (long i = 0; i < size; i++) {
				deckArray[i] = (rand() % optionsLeft) + 1;
			}
			delete[] optionsSwap;
			delete[] option;
		}

		void autoDeck(long inputSize){
			generateTimeSeed();
			fillDeck(inputSize);
		}

		long returnDeckI (long pos){
			return deckArray[pos];
		}
};
...
A deck of cards can be represented by an array of integers from 1 to 52.
do this:
Create a file called Permutation.h that will hold your function prototypes.
To accomplish our lab you will need to create three functions - but only two need to be in the header (Why?)
...
Code to implement these should be in your Permutations.cpp file.
