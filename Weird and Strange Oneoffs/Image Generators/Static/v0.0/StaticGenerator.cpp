	...
	for (int_fast32_t i = 0; i < 120; i++) {
		for (int_fast32_t y = 0; y < resolution; y++){
			for (int_fast32_t x = 0; x < resolution; x++){
				...
				//cout << "(" << (int) Temp.Red << ","<< (int) Temp.Green << "," << (int) Temp.Blue << "," << (int) Temp.Alpha << ")" << endl;
				AnImage.SetPixel(y,x,Temp);
				//writeArray << generator() % maxValue << endl;
			}
		}
		cout << i << " / 120" << endl;
		filenamemaker = "staticBoi_Frame" + to_string(i + 1) + ".bmp";
		AnImage.WriteToFile(filenamemaker.c_str());
	}
	return 0;
}
